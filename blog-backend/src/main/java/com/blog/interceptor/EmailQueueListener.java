...
import com.blog.domain.email.LeaveWordEmail;
import com.blog.domain.email.ReplyCommentEmail;
...
public class EmailQueueListener {
    ...
    public void handlerMapMessage(Map<String, Object> data) {
        ...
        if (type.equals(MailboxAlertsEnum.COMMENT_NOTIFICATION_EMAIL.getCodeStr())) {
            // 准备数据
            commentEmail = emailInformMapper
                    .getCommentEmailOne(String.valueOf(data.get("commentId")), (Integer) data.get("commentType"));
            String url = Objects.equals(commentEmail.getType(), CommentTypeEnum.ARTICLE_COMMENT.getType()) ?
                    webIndexPath + "article/" + commentEmail.getTypeId() : webIndexPath + "message/detail/" + commentEmail.getTypeId();
        ...
        if (type.equals(MailboxAlertsEnum.REPLY_COMMENT_NOTIFICATION_EMAIL.getCodeStr())) {
            // 回复评论数据
            CommentEmail commentEmailOne = emailInformMapper
                    .getCommentEmailOne(String.valueOf(data.get("commentId")), (Integer) data.get("commentType"));
            // 被回复评论数据
            Comment replyComment = commentMapper.
                    selectOne(new LambdaQueryWrapper<Comment>().eq(Comment::getId, data.get("replyCommentId")));
            User user = userMapper
                    .selectOne(new LambdaQueryWrapper<User>()
                            .eq(User::getId, replyComment.getCommentUserId()));
            replyCommentEmail = commentEmailOne.asViewObject(ReplyCommentEmail.class, reply -> {
                ...
                String url = Objects.equals(commentEmailOne.getType(), 1) ?
                        webIndexPath + "article/" + commentEmailOne.getTypeId() : webIndexPath + "message/detail/" + commentEmailOne.getTypeId();
                reply.setUrl(url);
            });
            // 如果是留言 title就为null
        ...
        if (type.equals(MailboxAlertsEnum.MESSAGE_NOTIFICATION_EMAIL.getCodeStr())) {
            LeaveWord messageUser = leaveWordMapper
                    .selectOne(new LambdaQueryWrapper<LeaveWord>()
                            .eq(LeaveWord::getId, data.get("messageId")));
            User user = userMapper
                    .selectOne(new LambdaQueryWrapper<User>()
                            .eq(User::getId, messageUser.getUserId()));
        ...
        if (MailboxAlertsEnum.REGISTER.getCodeStr().equals(type)) {
            mimeMessage = sendHtmlMail(email, MailboxAlertsEnum.REGISTER.getSubject(), MailboxAlertsEnum.REGISTER.getTemplateName(), Map.of(
                    ...
                    "code", code,
                    "toUrl", webIndexPath,
                    "openSourceAddress", "https://gitee.com/haibaraiii/snzh"
            ));
